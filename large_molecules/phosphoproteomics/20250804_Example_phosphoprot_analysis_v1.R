# MSF R Data Analysis for Phosphoproteomics v2.0 (updated 08/04/2025)
# -------------------------------------------------------------
# Project: Project Name (PI Name -- Institution)
# Date: 8/04/2025
# Experiment: Full Analysis of Phosphoproteomics

# Obj: Determine the response of phosphoproteins in tissue
# 1. Annotate full proteomics data (pg_matrix) with the experimental annotation file & annotate the phosphoproteomics data with its annotation file
# 2. Adjust the intensities of the phosphoproteomics data with the full proteomics data to account for differences in overall protein abundance
# 3. Apply normalization (if desired) to the adjusted phosphoproteomics data
# 4. Perform statistical analysis for each of the indicated paired groups
# 5. (If applicable) perform GO-SEA analysis


# Indicate the phosphoproteomics pg matrix
phospho_pg_matrix <- "phospho_report.pg_matrix.tsv"

# Indicate the phosphoproteomics exoeriment annotation file
phospho_annot <- "phospho_experiment_annotation.txt"
...
prefix <- "20250804_Example_Data_prot_"
l2fc_max <- 1
l2fc_min <- -1
pval_max <- 0.05
adj_pval_max <- 0.05
...
# Indicate in the quotes below which, if any, normalization methods you would like to apply to the adjusted phosphoproteomics
# options: "none", "medians", "subtraction", "vsn"
# Description of each:
# none: No normalization method is applied. Instead only the adjusted phosphoproteomics is used for downstream statistical analysis
# medians: Equalizing the medians will use the normalizeBetweenArrays function from the limma package
# subtraction: Normalization is applied by subtracting the median of each sample from the individual protein intensities
# vsn: Variance stabilizing normalization from the vsn package to remove variance related to the sample's mean
normalization <- "vsn"

# Select which conditions are to be analyzed from the phosphoproteomics in the list below. Ensure all conditions are spelled exactly as they're found in the annotation file, they are within quotes, and each condition separated by a comma
selected_conditions <- c("Mock 4 DPI", "Mock 6 DPI", "Treated 4 DPI", "Treated 6 DPI", "Pool")
...
# load phosphoproteomics annotations
p_annot <- setDT(read.delim(phospho_annot))

# load phosphoproteomics data
p_data <- setDT(read.delim(phospho_pg_matrix))
...
p_data <- melt(p_data, 1:5, variable.name = "sample", value.name = "intensity")
...
p_annot[, sample := gsub(":|\\\\|-",".", sample)]#[, sample := gsub("..", "X..",sample, fixed = T)]
...
p_data[p_annot, ":="(condition = i.condition, replicate = i.replicate), on = .(sample)]


#============================================================================
# PHOSPHOPROTEOMICS ADJUSTMENT
#============================================================================
# determine the median of the full proteomics data per sample group (Note: empty cells are ignored)
prot_med <- full_data[, .(med_int = median(intensity, na.rm = T)), by = .(Protein.Group, Protein.Ids, Genes, condition)]

# adjust phosphoproteomics to the median of the full proteomics
p_data[prot_med, adj_int := intensity * (intensity / i.med_int), on = .(Protein.Group, Protein.Ids, Genes, condition)]
p_data[is.na(adj_int), adj_int := intensity] # this will copy the unadjusted phosphoprotein intensities if no corresponding protein was found in the full data set


#============================================================================
# PHOSPHOPROTEOMICS NORMALIZATION
#============================================================================
# create matrix of samples/replicates vs. proteins
p_data[, sample_name := paste0(condition,"_",replicate)]
p_data_matrix <- dcast(p_data, Protein.Group ~ sample_name, value.var = "adj_int")
prot_label <- p_data_matrix$Protein.Group
p_data_matrix <- as.matrix(p_data_matrix[, 2:ncol(p_data_matrix)])
row.names(p_data_matrix) <- prot_label

# log2 transform intensities
p_data_matrix <- apply(p_data_matrix, c(1,2), log2)

if (normalization == "none") {
  
  p_data <- p_data
  
} else if (normalization == "medians") {
  
  library(limma)
  
  # normalize with limma package
  p_data_matrix <- normalizeBetweenArrays(p_data_matrix)
  
  
} else if (normalization == "subtraction") {
  
  # normalize by subtracting the median
  p_data_matrix <- p_data_matrix - robustbase::colMedians(p_data_matrix, na.rm = T)[col(p_data_matrix)]
  
} else if (normalization == "vsn") {
  
  library(vsn)
  
  # normalize using vsn package
  # remove the log2 transformation, as the vsn function will do this
  p_data_matrix <- apply(p_data_matrix, c(1,2), function(x){2^x})
  
  p_data_matrix <- justvsn(p_data_matrix)
  
} else if (!(normalization %in% c("none", "medians", "subtraction", "vsn"))) {
  
  rstudioapi::showDialog("Error", "Selected normalization is not recognized. Please go to line 18 and enter one of the following: \"none\", \"medians\", \"subtraction\", \"vsn\"")
  
}

# recombine with original data table
p_data_matrix <- as.data.table(p_data_matrix)
p_data_matrix <- data.table(p_data_matrix, Protein.Group = prot_label)
setcolorder(p_data_matrix, "Protein.Group")
p_data_matrix <- melt(p_data_matrix, 1, variable.name = "sample_name", value.name = "norm_int")
p_data[p_data_matrix, norm_int := i.norm_int, on = .(sample_name, Protein.Group)] # Note: if "none" was selected for normalization, then this will simply copy over the data
...
p_data <- p_data[(condition %in% selected_conditions), ]


#============================================================================
# STATISTICAL ANALYSIS OF PHOSPHOPROTEOMICS
#============================================================================
# load libraries
library(gplots)
library(ggrepel)
...
# full_data[, ":="(norm_int = log2(intensity), sample_name = paste0(condition,"_",replicate))] # p_data is normalized above and sample name is already created
full_data_metabo <- dcast(p_data, sample_name + condition ~ Protein.Group, value.var = "norm_int") # script should be exactly the same as the full proteomics R workflow from this line onward
